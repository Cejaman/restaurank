...
type PostProps = {
  ...
  likes: number;
  comments: number;
  // 🟢 1. AGREGAMOS LA PROPIEDAD OPCIONAL onDelete
  onDelete?: () => void;
};
...
export default function PostCard({
  ...
  likes,
  comments,
  onDelete, // 🟢 2. RECIBIMOS LA PROPIEDAD
}: PostProps) {
  ...
  const handleRestaurantPress = () => {
      if (restaurantId) router.push(`/restaurantes/${restaurantId}`);
  };
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.headerContainer}>
          ...
          >
            <Image
              source={{ uri: userImage || "https://i.imgur.com/8Km9tLL.png" }}
              style={styles.avatar}
            />
            <View>
          ...
          {/* 🟢 3. BOTÓN DE ELIMINAR (Solo se muestra si pasamos onDelete) */}
          {onDelete && (
      ...
      {/* Restaurante */}
      {restaurant && (
        <TouchableOpacity style={styles.restaurantRow} onPress={handleRestaurantPress} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  // Header ajustado para soportar el botón de borrar a la derecha
  ...
  deleteBtn: { padding: 5 }, // Estilo para el botón de basura
